...
var counter = 0
  , list_count = 0;
var etfs = {}
  , on_loading_complete = function() {
      // group ETFs by countries
      var sum = {};

      for (var e in etfs) {
          for (var i = 0; i < etfs[e].countries.length; i++) {
              if (etfs[e].countries[i] === 'UK') {
                  etfs[e].countries[i] = 'GB';
              }

              if (typeof sum[etfs[e].countries[i]] == 'undefined') {
                  sum[etfs[e].countries[i]] = 0;
              }
              sum[etfs[e].countries[i]] += etfs[e].quantity;
          }
      }
...
// Load all the ETFs
$.getJSON(WS_URL + '/etf/list', function (etfs_list) {
    list_count =  etfs_list.length;

    for (var i = 0; i < list_count; i++) {
        $.getJSON(WS_URL + '/etf/desc/' + etfs_list[i], function (desc) {
            $.getJSON(WS_URL + '/etf/price/' + desc.isin, function (__data) {
                var price = 0;
                ...
                var tr_id = "etf-" + desc.isin;
                var line = '<tr id="' + tr_id + '" data-isin="' + desc.isin + '">'
                            + 	'<td class="etf-column name"><a href="javascript:void(0)" onclick="show_etf_info(this)" data-code="' + desc.isin + '">' + desc.name + '</a></td>'
                            + 	'<td class="etf-column sector">' + desc.sectors.join(', ') + '</td>'
                            + 	'<td class="etf-column country"><i>' + desc.countries.join(', ') + '</i></td>'
                            + 	'<td class="etf-column number">' + desc.quantity + '</td>'
                            + 	'<td class="etf-column price">' + price + '&euro;</td>'
...
var __id = setInterval(function() {
    if (list_count == counter) {
        ...
        console.log(counter, '/', list_count);
    }
}, 500);